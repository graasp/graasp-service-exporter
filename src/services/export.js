...
const generateEpub = async ({
  ...
  let content = sections.filter(section => section.title && section.content);

  Logger.debug(content[0].content[0]);
...
const saveEpub = async (page, mode, lang, username, sectionNumber = null) => {
  ...
  const backCover = { title: 'Back Cover', data: backCoverContent };

  // concatenate introduction and body
  const content = [backCover, introduction, ...body, tools].filter(
